...
SERVER_URL="${KEYCLOAK_SERVER_URL:-http://localhost:8180}"
...
if [[ -f "$REALM_FILE" ]]; then
  if docker exec "$CONTAINER_NAME" /opt/keycloak/bin/kcadm.sh get realms/eaf >/dev/null 2>&1; then
    echo "EAF realm already present."
  else
    docker exec "$CONTAINER_NAME" /opt/keycloak/bin/kcadm.sh create realms -f /opt/keycloak/data/import/test-realm.json >/dev/null
    echo "Imported EAF realm into Keycloak."
  fi
else
