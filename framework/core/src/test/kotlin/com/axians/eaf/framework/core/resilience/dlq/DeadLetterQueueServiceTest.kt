 ...
 * @since 1.0.0
 */
class DeadLetterQueueServiceTest : FunSpec({
    ...
    context("Storing Failed Operations") {
        test("should store failed command") {
            ...
            val counter = meterRegistry.find("eaf.dlq.entries")
    ...
    context("Querying DLQ Entries") {
        ...
        test("should filter by status") {
            ...
            val entry1 = service.storeFailed(
                OperationType.COMMAND,
                TestCommand("1"),
                RuntimeException("Error")
            )
            val entry2 = service.storeFailed(
                OperationType.COMMAND,
                TestCommand("2"),
                RuntimeException("Error")
            )
    ...
    context("Replaying Entries") {
        test("should mark entry as replayed") {
            ...
            // And: Metrics are recorded
            val counter = meterRegistry.find("eaf.dlq.replays")
                .tag("status", "success")
                .counter()
            counter shouldNotBe null
        }
