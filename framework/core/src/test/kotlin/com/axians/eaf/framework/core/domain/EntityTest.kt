...
class EntityTest :
    FunSpec({
        ...
        test("should be equal when same ID (different instances)") {
            ...
            entity1 shouldBe entity2 // Same ID, different attributes
        }

        test("should not be equal when different IDs") {
        ...
        test("should be equal to itself (reflexive)") {
        ...
        test("should work correctly in HashSet (no duplicates with same ID)") {
            val entity1 = TestEntity(id = TestId("id-123"), name = "Entity1", value = 100)
            val entity2 = TestEntity(id = TestId("id-123"), name = "Entity2", value = 200)
            val entity3 = TestEntity(id = TestId("id-456"), name = "Entity3", value = 300)

            val set = hashSetOf(entity1, entity2, entity3)
            set.size shouldBe 2 // entity1 and entity2 are duplicates
        }

        test("should work correctly in HashMap as key") {
            val entity1 = TestEntity(id = TestId("id-123"), name = "Entity1", value = 100)
            val entity2 = TestEntity(id = TestId("id-123"), name = "Entity2", value = 200)
            ...
            map.size shouldBe 1 // Same key (same ID)
            map[entity1] shouldBe "second" // Overwritten value
        }

        test("should maintain identity equality despite attribute changes") {
            ...
            entity1 shouldBe entity2 // Same ID, attributes don't matter for equality
        }

        test("should support different entity types with same ID structure") {
            class AnotherEntity(
                override val id: TestId,
