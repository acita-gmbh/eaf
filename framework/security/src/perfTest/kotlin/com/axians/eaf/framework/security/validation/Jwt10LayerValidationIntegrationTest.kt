...
class Jwt10LayerValidationIntegrationTest {
    @Autowired
    ...
    @Nested
    inner class `All 10 Layers - Success Path` {
        @Test
        fun `valid JWT should pass all 10 validation layers and return 200 OK`() {
            ...
            assertThat(durationMs).isLessThan(200.0) // Allow overhead for Spring context

            // Verify SecurityContext was populated (AC3)
            val responseBody = result.response.contentAsString
            assertThat(responseBody).isEqualTo("""{"result":"success"}""")
        }
    }

    @Nested
    inner class `Layer 1-2 - Format and Signature Validation` {
        @Test
        fun `malformed JWT (missing parts) should be rejected`() {
        ...
        @Test
        fun `JWT with invalid signature should be rejected`() {
    ...
    @Nested
    inner class `Layer 3 - Algorithm Validation` {
        @Test
        fun `JWT with HS256 algorithm should be rejected (algorithm confusion attack)`() {
    ...
    @Nested
    inner class `Layer 4 - Claim Schema Validation` {
        @Test
        fun `JWT missing required claims should be rejected`() {
    ...
    @Nested
    inner class `Layer 5 - Time-based Validation` {
        @Test
        fun `expired JWT should be rejected`() {
    ...
    @Nested
    inner class `Layer 6 - Issuer and Audience Validation` {
        @Test
        fun `JWT with wrong issuer should be rejected`() {
        ...
        @Test
        fun `JWT with wrong audience should be rejected`() {
    ...
    @Nested
    inner class `Layer 7 - Token Revocation Check` {
        @Test
        fun `revoked JWT should be rejected`() {
        ...
        @Test
        fun `non-revoked JWT should pass revocation check`() {
    ...
    @Nested
    inner class `Layer 8 - Role Normalization` {
        @Test
        fun `JWT with valid roles should be normalized correctly`() {
    ...
    @Nested
    inner class `Layer 9 - User Validation (Optional)` {
        @Test
        fun `user validation should be skipped when disabled (default)`() {
    ...
    @Nested
    inner class `Layer 10 - Injection Detection` {
        @Test
        fun `JWT with injection patterns should be rejected`() {
    ...
    @Nested
    inner class `Metrics Validation` {
        @Test
        fun `metrics should be emitted for successful validation`() {
        ...
        @Test
        fun `failure metrics should be emitted on validation failure`() {
    ...
    @Nested
    inner class `Performance Validation` {
        @Test
        fun `validation should complete in under 50ms for valid JWT`() {
            // AC6: Performance validated <50ms total validation time
            val jwt = KeycloakTestContainer.generateToken("admin", "password")
