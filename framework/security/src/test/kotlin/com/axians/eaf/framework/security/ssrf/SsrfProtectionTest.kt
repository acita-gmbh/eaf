 ...
 * @since 1.0.0
 */
class SsrfProtectionTest : FunSpec({
    ...
    context("Scheme Validation") {
        ...
        test("should block non-HTTP schemes") {
            ...
            val exception = shouldThrow<SsrfException> {
    ...
    context("Host Whitelist") {
        test("should allow only whitelisted hosts") {
            // Given: SSRF protection with host whitelist
            val ssrf = SsrfProtection(
                SsrfProtectionProperties(
                    allowedHosts = setOf("api.example.com", "*.public.com")
                )
            )

            // When/Then: Whitelisted host should be allowed
            shouldNotThrow<SsrfException> {
                ssrf.validateUrl("https://api.example.com/data")
            }
        ...
        test("should support wildcard hosts") {
            ...
            // When/Then: Subdomain should be allowed
            shouldNotThrow<SsrfException> {
                ssrf.validateUrl("https://api.example.com/data")
            }
            shouldNotThrow<SsrfException> {
                ssrf.validateUrl("https://web.example.com/data")
            }
        }
    }
    ...
    context("IP Range Allowlist") {
        test("should allow only IPs in allowed ranges") {
            // Given: SSRF protection with IP allowlist
            val ssrf = SsrfProtection(
                SsrfProtectionProperties(
                    allowedIpRanges = setOf("203.0.113.0/24"),
                    allowedHosts = setOf("203.0.113.10") // Allow this specific IP
                )
            )

            // When/Then: IP in allowed range should be allowed
            shouldNotThrow<SsrfException> {
                ssrf.validateUrl("http://203.0.113.10/api")
            }
        }
    }
