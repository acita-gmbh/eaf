...
// dcm-infrastructure: Adapters, Projections, External integrations
...
val dcmMigrationsPath: String = projectDir.resolve("src/main/resources/db/migration").absolutePath
...
// 2. Runs Flyway migrations from both EAF and DCM
...
val generateJooqWithTestcontainers by tasks.registering {
    ...
    inputs.files(
        ...
        fileTree(dcmMigrationsPath) { include("*.sql") }
    )
                    ...
                    org.jooq.meta.jaxb.Generator()
                        ...
                        .withTarget(
                            org.jooq.meta.jaxb.Target()
                                .withPackageName("de.acci.dcm.infrastructure.jooq")
...
// and is tested via integration tests in dcm-app with Testcontainers.
...
kover {
    currentProject {
        instrumentation {
            ...
            // All adapters/repositories are tested via integration tests in dcm-app
            includedClasses.addAll(
                "de.acci.dcm.infrastructure.vmware.SpringSecurityCredentialEncryptor",
                "de.acci.dcm.infrastructure.vmware.SpringSecurityCredentialEncryptor\$*"
            )
        }
    }
