...
class PreCommitHooksConventionPlugin : Plugin<Project> {
    ...
    private fun Project.configurePreCommitHookTasks() {
        ...
        tasks.register("installGitHooks") {
            ...
            doLast {
                ...
                val templatesDir = File(rootDir, ".git-hooks")
                if (!templatesDir.exists()) {
                    throw IllegalStateException(".git-hooks directory not found – run scripts/install-git-hooks.sh once or ensure templates are checked in")
                }

                copyTemplateHook(templatesDir, hooksDir, "pre-commit", logger)
                copyTemplateHook(templatesDir, hooksDir, "pre-push", logger)
    ...
    private fun copyTemplateHook(templatesDir: File, hooksDir: File, hookName: String, logger: org.gradle.api.logging.Logger) {
        val template = templatesDir.resolve(hookName)
        if (!template.exists()) {
            throw IllegalStateException("Template ${template.absolutePath} not found. Run scripts/install-git-hooks.sh or add the template before invoking installGitHooks.")
        }
